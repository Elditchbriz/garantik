...
export default function SettingsPage() {
  ...
  // global de défilement (dans App.jsx, basé sur location.pathname) ne se
  // déclenche pas. On le fait ici aussi, spécifiquement pour ces onglets.
  useEffect(() => {
...
// Calcule les paliers de rappel réels (voir send-alerts.ts) à partir du
// nombre de mois choisi par l'utilisateur, pour affichage explicatif.
function getAlertThresholds(alertMonthsBefore) {
  const alertDays = alertMonthsBefore * 30;
  const thresholds = Array.from(new Set([alertDays, 30, 15, 7, 3, 1, 0].filter((t) => t <= alertDays)))
    .sort((a, b) => b - a);
  return thresholds;
}

function formatThreshold(days) {
...
function PreferencesTab({ orgId }) {
  ...
  async function handleSave() {
    ...
    await supabase.from('settings').update({
      ...
      product_emails_enabled: settings.product_emails_enabled !== false,
  ...
  const thresholds = getAlertThresholds(settings.alert_months_before);
  ...
  return (
    <>
      ...
      <div className="panel" style={{ marginBottom: 16 }}>
        ...
        {/* Explication des paliers de rappel — évite l'effet "un email par jour" */}
