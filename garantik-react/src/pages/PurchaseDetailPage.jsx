...
function DocumentViewer({ viewer, onClose, onDownload }) {
  ...
  return (
    <div ref={trapRef} tabIndex={-1} style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', background: 'rgba(0,0,0,0.5)' }}>
        ...
        <div style={{ display: 'flex', gap: 4, marginLeft: 16 }}>
          ...
          <button onClick={onClose} style={{ ...DOC_BTN, color: '#fff', fontSize: 22, lineHeight: 1 }}>
            <Icon name="x" />
          </button>
        </div>
      </div>
      <div style={{ flex: 1, overflow: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
        {viewer.type?.startsWith('image/') ? (
          ...
          <div style={{ color: '#fff', textAlign: 'center' }}>
            ...
            <button onClick={() => onDownload(viewer)} className="btn btn-primary" style={{ marginTop: 16 }}>Télécharger</button>
          </div>
        )}
      </div>
    </div>
...
export default function PurchaseDetailPage() {
  ...
  // Même raison que dans SettingsPage.jsx : changer d'onglet ici ne change
  // pas l'URL, donc le correctif global de défilement ne se déclenche pas.
  useEffect(() => {
  ...
  async function toggleAlertDismissed() {
    const next = !purchase.alert_dismissed;
    await supabase.from('purchases').update({ alert_dismissed: next }).eq('id', id);
  ...
  async function openViewer(doc) {
    ...
    // ouvre dans le lecteur système à la place. Les images restent dans
    // le visualiseur intégré, qui fonctionne très bien pour elles.
      ...
      {tab === 'detail' && (
        <>
          ...
          <div className="panel" style={{ marginBottom: 16, padding: '20px 20px 16px' }}>
            ...
            {/* Utile pour un vieil achat (ex: TV de 10 ans, garantie 2 ans) —
                évite d'être notifié "expirée" indéfiniment une fois que ce
                n'est plus pertinent de le savoir. */}
            {expired && (
              <div style={{ textAlign: 'center', marginTop: 12 }}>
                <button
                  ...
                  style={{
                    ...
                    fontSize: 12.5, fontWeight: 600, color: purchase.alert_dismissed ? 'var(--ink-faint)' : 'var(--blue)',
                    display: 'inline-flex', alignItems: 'center', gap: 6,
                  }}
                >
                  <Icon name={purchase.alert_dismissed ? 'bell' : 'bell-off'} style={{ fontSize: 13 }} />
                  {purchase.alert_dismissed ? 'Alertes coupées — réactiver' : 'Ne plus m\'alerter sur cet article'}
                </button>
              </div>
            )}
          </div>
          ...
          <div className="panel" style={{ marginBottom: 16 }}>
            <div className="panel-header">
              ...
              <button onClick={() => setShowLinkModal(true)} className="btn btn-primary" style={{ fontSize: 12, padding: '6px 12px', flexShrink: 0 }}>
                <Icon name="plus" /> Lier
              </button>
            </div>
            {contracts.length === 0 ? (
            ...
            <div className="panel">
              ...
              <div className="panel-body" style={{ padding: 0 }}>
                {documents.map(doc => {
                  ...
                  const menuOpen = openDocMenu?.docId === doc.id;
                  return (
