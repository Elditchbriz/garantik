              ...
              <div className="panel-body" style={{ padding: 0 }}>
                {documents.map(doc => (
                  <div key={doc.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderBottom: '1px solid var(--line)' }}>
                    ...
                    <button onClick={() => openViewer(doc)} title="Visualiser" style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--blue)', padding: 6 }}><Icon name="eye" /></button>
                    <button onClick={() => downloadFile(doc)} title="Télécharger" style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--ink-soft)', padding: 6 }}><Icon name="download" /></button>
                    <button onClick={() => handleDeleteDoc(doc.id, doc.file_path)} title="Supprimer" style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--red-text)', padding: 6 }}><Icon name="x" /></button>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', zIndex: 2000, display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', background: 'rgba(0,0,0,0.5)' }}>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              <button onClick={() => downloadFile(viewer)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#fff', fontSize: 13, display: 'flex', alignItems: 'center', gap: 6 }}><Icon name="download" /> Télécharger</button>
