...
const BTN = { background: 'none', border: 'none', cursor: 'pointer', padding: '6px 8px', borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center' };
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.9)', zIndex: 2000, display: 'flex', flexDirection: 'column' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 20px', background: 'rgba(0,0,0,0.5)' }}>
            ...
            <div style={{ display: 'flex', gap: 12, marginLeft: 16 }}>
              <button onClick={() => downloadFile(viewer)} style={{ ...BTN, color: '#fff', fontSize: 13, gap: 6 }}>
                <Icon name="download" /> Télécharger
              </button>
              <button onClick={() => setViewer(null)} style={{ ...BTN, color: '#fff', fontSize: 22 }}>
                <Icon name="x" />
              </button>
            </div>
          </div>
          <div style={{ flex: 1, overflow: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}>
            {viewer.type?.startsWith('image/') ? (
              ...
              <button onClick={() => downloadFile(viewer)} className="btn btn-primary">Télécharger</button>
            )}
          </div>
        </div>
      )}
