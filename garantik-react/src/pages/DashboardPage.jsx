...
export default function DashboardPage() {
  ...
  const [showOnboarding, setShowOnboarding] = useState(
    !localStorage.getItem('garantik_onboarding_done')
  );
  ...
  function dismissOnboarding() {
    localStorage.setItem('garantik_onboarding_done', '1');
    ...
    <>
      ...
      <div className="dash-add-row">
        <button className="dash-add-btn primary" onClick={() => navigate('/add-purchase')}>
          <div className="dash-add-icon"><Icon name="shopping-bag" /></div>
          <div>
        ...
        <button className="dash-add-btn secondary" onClick={() => navigate('/add-contract')}>
          <div className="dash-add-icon"><Icon name="file-certificate" /></div>
          <div>
      ...
      <div className="dash-stats">
        {[
          { label: 'Total',     num: stats.all + contracts.length,   icon: 'stack',          color: 'blue' },
          { label: 'Actifs',    num: stats.active + contracts.filter(c => itemStatus(c.end_date) === 'active').length,   icon: 'circle-check',   color: 'green' },
          { label: 'Bientôt',   num: expiringSoon,                   icon: 'clock',          color: 'amber' },
          { label: 'Expirés',   num: stats.expired + contracts.filter(c => itemStatus(c.end_date) === 'expired').length, icon: 'alert-triangle', color: 'red' },
        ].map(s => (
