...
export default function AddContractPage() {
  ...
  async function handleSubmit(e) {
    ...
    if (data?.id && scannedBlob) {
      await uploadDocument(scannedBlob, orgId, null, `contrat_${name.replace(/[^\p{L}\p{N}]+/gu, '-')}.jpg`, data.id);
      await supabase.from('documents').update({ document_category: 'contrat' }).eq('contract_id', data.id);
    }
    ...
    <>
      <div className="topbar">
        <div>
          ...
          <p className="sub">Extension de garantie, assurance, abonnement ou autre</p>
        </div>
      </div>
