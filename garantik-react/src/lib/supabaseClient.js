...
export async function uploadDocument(file, organizationId, purchaseId = null, customName = null, contractId = null) {
  ...
  if (driveConn) {
    // Convertir le fichier en base64 (méthode compatible avec les gros fichiers)
    ...
    if (!driveError && driveData?.document_id) {
      // L'Edge Function a créé et mis à jour le document — on le récupère
      const { data: docData } = await supabase
        .from('documents')
        .select('*')
        .eq('id', driveData.document_id)
        .single();
      return { data: docData, error: null };
    }
    console.error('Erreur upload Drive, fallback Supabase:', driveError);
