    ...
    <div className="field" style={{ position: 'relative' }}>
      ...
      {isNewValue && !added && (hasCloseMatch || true) && (
        <div style={{
          ...
          {hasCloseMatch && (
            <>
              ...
              {similarItems.slice(0, 3).map(item => (
                <div key={item.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6, flexWrap: 'wrap', gap: 6 }}>
          ...
          {/* flex-wrap ajouté : sur une colonne étroite (formulaires en
              grille 2 colonnes), le texte + bouton se chevauchaient sans
              jamais retomber à la ligne. */}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, flexWrap: 'wrap' }}>
            <span style={{ fontSize: 12.5, color: hasCloseMatch ? 'var(--ink-soft)' : 'var(--blue-dark)', minWidth: 0 }}>
              {hasCloseMatch
